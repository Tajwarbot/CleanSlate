...
| **Messenger DOM Adapter** | `src/adapters/messenger-adapter.ts` | ✅ Complete | Current-chat message discovery, oldest-message loading, sequential unsend/remove actions, confirmation handling, verification |
...
9. **User-Controlled Lazy Loading and Native Bulk Removal**: Facebook Activity Log entries are lazy-loaded, so the content script scrolls the document and relevant inner scroll containers while writing progress after each pass. It compares both page height and rendered item count, requiring twelve consecutive stable passes before treating loading as finished and allowing up to 1,000 scroll passes, while allowing the user to stop at the current batch. Facebook categories then use the page's native `All`, `Remove`, and confirmation controls for the loaded batch; the cleanup phase does not start a second full-page load. The UI calls the old dry-run behavior **Safety Preview**: it may select visible activity but never clicks `Remove`. Messenger stays isolated in its adapter: it targets only the open conversation, finds Messenger's role-based message rows, scrolls the conversation viewport to the oldest loaded history, then processes visible messages sequentially with explicit `More` → `Remove/Unsend` → confirmation actions and pauses between requests.
   ...
   If the requested category heading and native **All** control are already rendered, scanning must not click a similarly named navigation item; Facebook can route that click through Home or the default Activity Log.
   Scan progress records remain in `scanning` state until the content script explicitly writes `complete` with discovered items. The popup must not use a wall-clock timeout to return to the dashboard: Facebook scans can legitimately take longer than five minutes. A new scan overwrites the previous record, and explicit dashboard navigation clears the guided handoff.
   Loading controls are distinct: **Stop here and review this batch** preserves the currently rendered batch and completes a partial scan; **Cancel scan and return to dashboard** writes a cancel control, prevents the content script from writing completion, and clears scan storage so the next popup open starts on the dashboard.

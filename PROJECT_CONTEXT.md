...
9. **User-Controlled Lazy Loading and Native Bulk Removal**: Facebook Activity Log entries are lazy-loaded, so the content script scrolls the document and relevant inner scroll containers while writing progress after each pass. The popup lets the user either stop at the currently rendered batch or continue loading until the page stabilizes. Facebook categories then use the page's native `All`, `Remove`, and confirmation controls for the loaded batch; the cleanup phase does not start a second full-page load. Dry Run may select visible activity but never clicks `Remove`; Messenger conversations continue using the per-item adapter flow.
